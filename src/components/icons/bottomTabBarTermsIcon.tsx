import React from 'react';
import { cn } from '@/lib/utils';

interface BottomTabBarIconProps extends React.SVGProps<SVGSVGElement> {
  color?: 'secondary1' | 'secondary2';
}

export const BottomTabBarTermsIcon = ({
  color = 'secondary2',
  className,
  ...props
}: BottomTabBarIconProps) => {
  const colorMap = {
    secondary1: 'text-secondary1',
    secondary2: 'text-secondary2',
  };

  return (
    <svg
      xmlns='http://www.w3.org/2000/svg'
      width='24'
      height='24'
      viewBox='0 0 24 24'
      fill='none'
      className={cn(colorMap[color], className)}
      {...props}
    >
      <path
        d='M21 12C21 13.78 20.4722 15.5201 19.4832 17.0001C18.4943 18.4802 17.0887 19.6337 15.4442 20.3149C13.7996 20.9961 11.99 21.1743 10.2442 20.8271C8.49836 20.4798 6.89471 19.6226 5.63604 18.364C4.37737 17.1053 3.5202 15.5016 3.17293 13.7558C2.82567 12.01 3.0039 10.2004 3.68508 8.55585C4.36627 6.91131 5.51983 5.50571 6.99987 4.51677C8.47991 3.52784 10.22 3 12 3'
        stroke='currentColor'
        strokeWidth='2'
        strokeLinejoin='round'
      />
      <path
        d='M13.1716 14.8284L19 9C20.1046 7.89543 20.1046 6.10457 19 5C17.8954 3.89543 16.1046 3.89543 15 5L9.17157 10.8284C8.42143 11.5786 8 12.596 8 13.6569V16H10.3431C11.404 16 12.4214 15.5786 13.1716 14.8284Z'
        stroke='currentColor'
        strokeWidth='2'
        strokeLinejoin='round'
      />
      <path
        d='M15 9C16.1046 10.1046 17.8954 10.1046 19 9C20.1046 7.89543 20.1046 6.10457 19 5C17.8954 3.89543 16.1046 3.89543 15 5C13.8954 6.10457 13.8954 7.89543 15 9Z'
        stroke='currentColor'
        strokeWidth='2'
        strokeLinejoin='round'
      />
    </svg>
  );
};
