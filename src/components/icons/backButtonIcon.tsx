...
const BackButtonIcon = ({ color = 'secondary2' }: BackButtonIconProps) => {
  const colorMap = {
    secondary1: 'text-secondary1',
    secondary2: 'text-secondary2',
  };

  const textColorClass = colorMap[color] || 'text-secondary2';

  return (
    <div
      className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-full border border-current ${textColorClass}`}
    >
      <svg
