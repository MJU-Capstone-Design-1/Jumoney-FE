...
export const getBacktestMasterMockHandler = (overrideResponse?: ApiResponseMasterChoiceBacktestResponse | ((info: Parameters<Parameters<typeof http.post>[1]>[0]) => Promise<ApiResponseMasterChoiceBacktestResponse> | ApiResponseMasterChoiceBacktestResponse), options?: RequestHandlerOptions) => {
  return http.post('*/api/master-choice/masters/:masterId/backtests/stocks/:stockCode', async (info: Parameters<Parameters<typeof http.post>[1]>[0]) => {


    return HttpResponse.json(overrideResponse !== undefined
    ? (typeof overrideResponse === "function" ? await overrideResponse(info) : overrideResponse)
    : getBacktestMasterResponseMock(),
      { status: 200
      })
  }, options)
}
